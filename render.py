...
#! nix-shell -i python3 -p "python3.withPackages(ps:[ps.pycairo ps.tqdm ps.tinycss2])"
...
def render(cr, frame, s, t, text, html, df):
    dpi = 400 / 72
    ...
    if html == None:
        ...
        for r in range(len(parsed)):
            ...
            for block in blocks:
                ...
                cr.move_to(c,imagesize[1] - bottom_margin + (r - current_row + 1) * lineheight)
                cr.show_text(text)
...
#for frame in tqdm.tqdm(range(subset,subset+500)):
for frame in tqdm.tqdm(range(duration)):
    ...
    if loaded_html == loaded_text:
        render(cr, frame, s, t, text, html, df)
    else:
        render(cr, frame, s, t, text, None, df)
