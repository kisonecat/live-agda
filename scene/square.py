...
def render(cr, width, height, t, line, word, text, row, column):
    global square_alpha, terms, arrows_alpha, i_alpha, j_alpha, filler_alpha
    ...
    if word == 'i' or word == '(i':
        i_alpha = (i_alpha + 0.4)/2
    else:
        i_alpha = (i_alpha + 0)/2

    if word == 'j' or word == 'j)':
        j_alpha = (j_alpha + 0.4)/2
    else:
        j_alpha = (j_alpha + 0)/2

    if word == 'hfill' or word == 'Square':
        filler_alpha = (filler_alpha + 0.4)/2
    else:
        filler_alpha = (filler_alpha + 0)/2
        
    if 'Square' in paragraph:
    ...
    words = line.split(' ')
    if 'Square' in words:
        terms = words[words.index('Square')+1:]

        word_column = len(line[0:column].split(' '))
        j = word_column - words.index('Square') - 2
        ...
        j = -1
        if 'i = i0' in line and column > 14 and word != 'hfill':
            j = 0
        if 'i = i1' in line:
            j = 1
        if 'p i' in line:
    ...
    cr.set_source_rgba(38/255, 139/255, 210/255, square_alpha*0.2 + filler_alpha) 
    ...
    if len(terms) >= 4:
        cr.set_source_rgba(1.1*38/255, 1.1*139/255, 1.1*210/255, square_alpha * (0.5 + terms_alpha[3]))
